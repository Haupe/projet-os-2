...
int good_execution(int socket, char* query);
...
int good_execution(int socket, char* query) {
	...
	pthread_mutex_lock(&query_mutex.new_access);
	pthread_mutex_lock(&query_mutex.write_access);
	pthread_mutex_unlock(&query_mutex.new_access);
	if (strncmp("update", query, sizeof("update")-1) == 0) {
    	parse_and_execute_update(socket, &database, query);
		result = 1;
  	} else if (strncmp("insert", query, sizeof("insert")-1) == 0) {
    	parse_and_execute_insert(socket, &database, query);
		result = 1;
  	} else if (strncmp("delete", query, sizeof("delete")-1) == 0) {
    	parse_and_execute_delete(socket, &database, query);
		result = 1;
	}
	pthread_mutex_unlock(&query_mutex.write_access);
	pthread_mutex_lock(&query_mutex.new_access);
	pthread_mutex_lock(&query_mutex.reader_registration);
	if (query_mutex.readers_c == 0){
		pthread_mutex_lock(&query_mutex.write_access);
	}
	query_mutex.readers_c++;
	pthread_mutex_unlock(&query_mutex.new_access);
	pthread_mutex_unlock(&query_mutex.reader_registration);
	if (strncmp("select", query, sizeof("select")-1) == 0) {

		parse_and_execute_select(socket, &database, query);
		result = 1;
	}
	pthread_mutex_lock(&query_mutex.reader_registration);
	query_mutex.readers_c --;
...
void *client(int socket){
	...
	while ((lu = read(socket, query, 256)) > 0){
		...
		if(good_execution(socket, query)==0){
			query_fail_bad_query_type(socket);
		}
